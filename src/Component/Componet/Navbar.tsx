...
const [sidebarOpen, setSidebarOpen]=useState(false)
    ...
    return (
        <nav className="w-full py-7   flex justify-between mx-auto items-center horizantalPadding ">
            {/* <div className={`fixed  top-0 w-300 z-40 ${sidebarOpen===false ?"translate-x-0":"translate-x-300"}`} >
            ...
            <div className="flex gap-6">
                <div className="hidden lg:flex gap-6 py-5 ">
                    {["Home", "About Us", "Our Companies", "Contact US"].map((items, index) => (
                        ...
                        >
                            ...
                            {downarrow.includes(items) && <MdOutlineKeyboardArrowDown style={{ fontSize: "24px" }} />}
                                ...
                                <button className='hidden lg:flex items-center space-x-3 bg-gradient-to-r from-blue-400 to-blue-600 text-white px-6  py-5 font-semibold rounded-md   shadow-md     hover:shadow-lg transition'>
                    <span>ADD LISTING</span>
                    <button ><IoArrowForwardSharp/></button>
                </button>
                <button
                    onClick={() => setSidebarOpen(!sidebarOpen)}
                    className="lg:hidden px-3  text-gray-500"><GiHamburgerMenu size={25} /></button>
            </div>
